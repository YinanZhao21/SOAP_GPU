...
def Line_removal(wavelength, spec_seed_spot):
    ...
    line_list = np.array([5250.2084, 5250.6453, 5434.5232, 5432.9470, 5576.0881])
...
for i_t in np.arange(T_eff.size):
    ...
    if spectral_type[i_t] == 'quiet':

        spec_seed =  np.loadtxt(output_dir+'/solar_spec.txt')[:vector_size,1]


    else:
        spec_seed =  np.loadtxt(output_dir+'/solar_spot_spec.txt')[:vector_size,1]*1.1431532695663713
        index = spec_seed > 1.0
        spec_seed[index] = 1.0
        print('use active region')
